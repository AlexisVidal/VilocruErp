...
namespace ERP.Admin.Controllers
{
    ...
    public class NotaCreditoController : Controller
    {
        ...
        public async Task<NotaCredito> InsertNotaCredito(NotaCredito nota_credito)
        {
            NotaCredito entidad = null;
            try
            {
                List<NotaCredito> lstEntidad = null;
                ...
                var response = await client.PostAsJsonAsync("NotaCredito/agregar", nota_credito);

                var model = response.Content.ReadAsAsync<List<NotaCredito>>();
        ...
        public async Task<NotaCredito> GetNotaCreditoById(int IdNotaCred)
        {
            NotaCredito entidad = null;
            try
            {
                ...
                NotaCredito parametro = new NotaCredito { id_nota_credito = IdNotaCred };
                ...
                var response = await client.PostAsJsonAsync("NotaCredito/buscar", parametro);
        ...
        public async Task<List<NotaCredito>> GetNotaCreditoAll()
        {
            List<NotaCredito> lstEntidad = null;
            HttpClient client = new HttpClient();
            try
            {
                ...
                HttpResponseMessage response = await client.GetAsync("NotaCredito/all");
                var model = response.Content.ReadAsAsync<List<NotaCredito>>();
                if (model.Result.Count > 0)
                {
                    lstEntidad = model.Result.ToList();
                ...
                {
                    lstEntidad = new List<Models.NotaCredito>();
                }
            }
            catch (Exception ex)
            {
        ...
        {
            List<NotaCredito> lstEntidad = null;
            lstEntidad = await GetNotaCreditoAll();
        ...
        public async Task<ActionResult> SaveNewNotaCredito(int FkCompVent, string NroCompNC, string FechEmisNC,
            ...
            string DescMotiNC, int MotiNC, string flgAfecStoc, string Sigla
            ,List<List<string>> arrNotaCredDeta
            )
        {
            string msgReturn = "";
            string DataQr = "";
            ...
            try
            {
                ...
                NotaCredito notaCreditoReturn = null;
                ...
                if (varcomprob != null)
                {
                    ...
                    notaCreditoReturn = await InsertNotaCredito(nota_credito);
                    if (notaCreditoReturn != null)
                    {
                        IdNewNotaCred = notaCreditoReturn.id_nota_credito;

                        if (arrNotaCredDeta != null && arrNotaCredDeta.Any())
                        {
                            flgReturnSave = await SaveListNotaCreditoDetalle(IdNewNotaCred, arrNotaCredDeta, flgAfecStoc);
                            if (flgReturnSave == 0)
                            {
                            ...
                            else
                            {
                                ...
                                if (rptaFactE.errors == null)
                                {
                                    ...
                                    try
                                    {
                                        NotaCredito xnota = new NotaCredito()
                                        {
                                            id_nota_credito = notaCreditoReturn.id_nota_credito,
                                            codigo_hash = rptaFactE.codigo_hash,
                                            cadena_para_codigo_qr = rptaFactE.cadena_para_codigo_qr,
                                            enlace = rptaFactE.enlace
                                        };
                                        var xyz = await UpdateNotaCreditoRespuesta(xnota);
                                    }
                                    catch (Exception ex)
                                    {
                                ...
                                {
                                    notaCreditoReturn = await SaveDeleteComprobanteVenta(notaCreditoReturn.id_nota_credito);
            ...
            //return Json(msgReturn, JsonRequestBehavior.AllowGet);
            return Json(new { msgRpta = msgReturn, IdNotaCredVent = IdNewNotaCred, DataQr = DataQr }, JsonRequestBehavior.AllowGet);
        }

        public async Task<NotaCredito> SaveDeleteComprobanteVenta(int IdCompVent)
        {
            NotaCredito comprobante_venta = null;
            try
            {
        ...
        public async Task<NotaCredito> DeleteComprobanteVentaId(int IdCompVent)
        {
            NotaCredito entidad = null;
            try
            {
                ...
                var client = new HttpClient();
                NotaCredito parametro = new NotaCredito { id_nota_credito = IdCompVent };
                string connectionInfo = System.Configuration.ConfigurationManager.AppSettings["Uristring"];
                client.BaseAddress = new Uri(connectionInfo);
                client.DefaultRequestHeaders.Accept.Clear();
                client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                var response = await client.PostAsJsonAsync("NotaCredito/eliminarid", parametro);
        ...
        public static async Task<NotaCredito> UpdateNotaCreditoRespuesta(NotaCredito nota)
        {
            NotaCredito entidad = null;
            try
            {
                ...
                client.DefaultRequestHeaders.Accept.Clear();
                client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                var response = await client.PostAsJsonAsync("NotaCredito/t_nota_creditoUpdateRespuesta", nota);
        ...
        public async Task<Respuesta> EnviaNCFacturacionElectronica(NotaCredito notaCreditoReturn)
        {
            List<NotaCreditoDetalle> lstNotaCreditoDetalle = null;
            ...
            NotaCreditoDetalleController CtrlNotaCredDeta = new NotaCreditoDetalleController();
            ...
            try
            {
                ...
                Items one_item = null;
                lstNotaCreditoDetalle = await CtrlNotaCredDeta.GetNotaCreditoDetalleByNotaCredito(notaCreditoReturn.id_nota_credito);
                if (lstNotaCreditoDetalle != null)
                {
                    lstNotaCreditoDetalle = lstNotaCreditoDetalle.Where(i => !i.estado.Equals("0")).ToList();
                }
                foreach (var oneItem in lstNotaCreditoDetalle)
                {
                    double factIgv = 1.18;
                ...
                invoice.tipo_de_nota_de_credito = notaCreditoReturn.motivo_nc;
                ...
                try
                {
                    ...
                    System.IO.File.WriteAllText(path + "notacredito-" + invoice.cliente_numero_de_documento + "-" + invoice.serie + ".json", jsondata);
                }
                catch (Exception ex)
                {
        ...
        public async Task<int> SaveListNotaCreditoDetalle(int FkNotaCred, List<List<string>> arrNotaCredDeta, string flgAfecStoc)
        {
            int flgSaveMovi = 0;
            NotaCreditoDetalleController CtrlNotaCredDeta = new NotaCreditoDetalleController();
            try
            {
                NotaCreditoDetalle notaCreditoDetalleReturn = null;
                NotaCreditoDetalle nota_credito_detalle = null;
                foreach (var oneDetaVent in arrNotaCredDeta)
                {
                    ...
                    notaCreditoDetalleReturn = await CtrlNotaCredDeta.InsertNotaCreditoDetalle(nota_credito_detalle);
        ...
        public async Task<int> SaveMovimiento(int FkNotaCredDeta, int FkProd, decimal Cant)
        {
            ...
            try
            {
                ...
                int FkMoviTipo = Convert.ToInt32(System.Configuration.ConfigurationManager.AppSettings["MoviTipo_IngresoNotaCredito"]);
        ...
        public async Task<ActionResult> Edit(int IdNotaCred, string CallBy)
        {
            ...
            NotaCredito nota_credito = null;
            List<NotaCreditoDetalle> lstNotaCreditoDetalle = new List<NotaCreditoDetalle>();
            List<ComprobanteTipo> lstComprobanteTipo = null;
            ...
            try
            {
                nota_credito = await GetNotaCreditoById(IdNotaCred);
                if (nota_credito != null)
                {
                    lstNotaCreditoDetalle = await CtrlNotaCredDeta.GetNotaCreditoDetalleByNotaCredito(IdNotaCred);
                    if (lstNotaCreditoDetalle != null)
                    {
                        lstNotaCreditoDetalle = lstNotaCreditoDetalle.Where(i => !i.estado.Equals("0")).ToList();
                    }
                ...
                ViewBag.ComprobanteTipo = lstComprobanteTipo;
                ViewBag.NotaCreditoDetalle = lstNotaCreditoDetalle;
                ViewBag.CallBy = CallBy;
                ViewBag.FkCompTipoNotaCredito = FkCompTipoNotaCred;
            }
            catch (Exception ex)
        ...
        public async Task<ActionResult> ViewNotaCreditoPendientes(string CallBy)
        {
            List<NotaCredito> lstEntidad = null;
            lstEntidad = await GetNotaCreditoAll();
            if (lstEntidad != null)
        ...
        public async Task<ActionResult> ExportComprobante(int IdNotaCredVent)
        {
            NotaCreditoDetalleController CtrlVentDeta = new NotaCreditoDetalleController();
            ...
            try
            {
                ...
                //NotaCredito nota_credito_venta = await GetNotaCreditoById(IdNotaCredVent);
                List<NotaCreditoDetalle> nota_credito_venta = await CtrlVentDeta.GetNotaCreditoDetalleByNotaCredito(IdNotaCredVent);
                TotaIgv = nota_credito_venta[0].total_igv.ToString("#,##0.00");
                TotaNeto = nota_credito_venta[0].total_neto.ToString("#,##0.00");
                string appPath = System.Web.HttpContext.Current.Request.ApplicationPath.ToLower();

                string SeriCV = nota_credito_venta[0].nro_comprobante_comprobante_venta.Substring(0, 4);
                //SeriCV = nota_credito_venta.sigla + SeriCV;
                DocuAfecta = nota_credito_venta[0].descripcion_comprobante_tipo + " " + SeriCV + "-" + nota_credito_venta[0].nro_comprobante_comprobante_venta.Substring(5, nota_credito_venta[0].nro_comprobante_comprobante_venta.Length - 5);

                string MotivoNC = nota_credito_venta[0].motivo_nc.ToString();
                string Motivo = nota_credito_venta[0].motivo;

                string SeriNC = nota_credito_venta[0].nro_comprobante.Substring(1, 3);
                SeriNC = nota_credito_venta[0].sigla + SeriNC;
                sigla = "NOTA DE CREDITO ELECTRONICA";
                string NroNC = SeriNC + "-" + nota_credito_venta[0].nro_comprobante.Substring(5, nota_credito_venta[0].nro_comprobante.Length - 5);

                TotalLetras = nota_credito_venta[0].total_bruto.NumeroALetras();
                string DniRucClie = "";
                string NombClie = "";
                string DireClie = "";
                if (nota_credito_venta[0].id_cliente_natural > 0)
                {
                    DniRucClie = nota_credito_venta[0].dni_cliente_natural;
                    NombClie = nota_credito_venta[0].nombre_cliente_natural + " " + nota_credito_venta[0].apellido_paterno_cliente_natural + " " + nota_credito_venta[0].apellido_materno_cliente_natural;
                    DireClie = nota_credito_venta[0].direccion_cliente_natural;
                }
                else
                {
                    DniRucClie = nota_credito_venta[0].ruc_empresa_cliente_juridico;
                    NombClie = nota_credito_venta[0].razon_social;
                    DireClie = nota_credito_venta[0].direccion_empresa;
                }
                //DataSet data = await Get_DataNotaCredito(IdNotaCredVent);
                //if (data?.Tables.Count > 0)
                //{
                ReportDocument rd = new ReportDocument();
                rd.Load(Path.Combine(Server.MapPath("~/img/reportes/CrNotaCreditoVenta.rpt")));
                rd.DataSourceConnections[0].SetConnection(dataurl, databases, datauser, dataclave);
                rd.SetParameterValue("@fk_nota_credito", IdNotaCredVent);

                QRCodeGenerator qrGenerator = new QRCodeGenerator();
                QRCodeData qrCodeData =
                    qrGenerator.CreateQrCode(nota_credito_venta[0].cadena_para_codigo_qr, QRCodeGenerator.ECCLevel.Q);
                ...
                rd.Close();
                rd.Dispose();
                return File(stream, "application/pdf", "NotaCredito-" + NroNC + ".pdf");
        ...
        public async Task<ActionResult> PrintNotaCredito(int IdNotaCredVent, string DataQr)
        {
            try
            {
                string DocuAfecta = "";
                ...
                //Obtengo el Comprobante Venta
                NotaCredito nota_credito_venta = await GetNotaCreditoById(IdNotaCredVent);
                TotaIgv = nota_credito_venta.total_igv.ToString("#,##0.00");
                TotaNeto = nota_credito_venta.total_neto.ToString("#,##0.00");
                string appPath = System.Web.HttpContext.Current.Request.ApplicationPath.ToLower();

                string SeriCV = nota_credito_venta.nro_comprobante_comprobante_venta.Substring(0, 4);
                //SeriCV = nota_credito_venta.sigla + SeriCV;
                DocuAfecta = nota_credito_venta.descripcion_comprobante_tipo + " " + SeriCV + "-" + nota_credito_venta.nro_comprobante_comprobante_venta.Substring(5, nota_credito_venta.nro_comprobante_comprobante_venta.Length - 5);

                string MotivoNC = nota_credito_venta.motivo_nc.ToString();
                string Motivo = nota_credito_venta.motivo;

                string SeriNC = nota_credito_venta.nro_comprobante.Substring(1, 3);
                SeriNC = nota_credito_venta.sigla + SeriNC;
                string NroNC = "NC - " + SeriNC + "-" + nota_credito_venta.nro_comprobante.Substring(5, nota_credito_venta.nro_comprobante.Length - 5);

                string DniRucClie = "";
                string NombClie = "";
                string DireClie = "";
                if (nota_credito_venta.id_cliente_natural > 0)
                {
                    DniRucClie = nota_credito_venta.dni_cliente_natural;
                    NombClie = nota_credito_venta.nombre_cliente_natural + " " + nota_credito_venta.apellido_paterno_cliente_natural + " " + nota_credito_venta.apellido_materno_cliente_natural;
                    DireClie = nota_credito_venta.direccion_cliente_natural;
                }
                else
                {
                    DniRucClie = nota_credito_venta.ruc_empresa_cliente_juridico;
                    NombClie = nota_credito_venta.razon_social;
                    DireClie = nota_credito_venta.direccion_empresa;
                }
                DataSet data = await Get_DataNotaCredito(IdNotaCredVent);
                if (data?.Tables.Count > 0)
                {
        ...
        public async Task<ActionResult> PrintNotaCreditoView(int IdNotaCredVent, string DataQr)
        {
            ...
            try
            {
                ...
                rd.ExportToDisk(ExportFormatType.PortableDocFormat, @"C:\reportesotros\NC-" + NroNC + ".pdf");

                ViewBag.Printer = "NC-" + NroNC;
                ViewBag.Codigo = "NC-" + NroNC + ".pdf";
        ...
        public async Task<DataSet> Get_DataNotaCredito(int FkNotaCred)
        {
            NotaCreditoDetalleController CtrlNotaCredDeta = new NotaCreditoDetalleController();
            var lstVentaDetalle = await CtrlNotaCredDeta.GetNotaCreditoDetalleByNotaCredito(FkNotaCred);
            if (lstVentaDetalle != null)
            {
